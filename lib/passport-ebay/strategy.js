...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query['tknexp'] && req.query['username']) {
    ...
    var tknexp = req.query.tknexp;ap
    ...
    var input = {
      ...
      params: {
        'authToken': this.options.ownertoken,
        // 'RuName': ruName,
        'SessionID': sessionID
      }
    };
    ...
    self._ebayClient.ebayApiPostXmlRequest(input, function(error, results) {
      ...
      var url = require('ebay-api').buildRequestUrl('Signin', {RuName : self.options.ruName, SessID : sessionID}, null, self.options.sand);
      url = util.format("https://signin.sandbox.ebay.com/ws/eBayISAPI.dll?SignIn&RuName=%s&SessID=%s&ruparams=signup", self.options.ruName, sessionID );
